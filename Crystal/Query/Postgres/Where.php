		...
		switch ($method) 
		{
						...
						if(is_array($parsed_params))
						{
							...
							{	
								...
								foreach($parsed_params as $k => $v)
								{
									...
									$this->where .= ' ' . Crystal_Helper_Postgres::sanitize_string($v['column']);
				  					$this->where .= ' ' . $v['param'];
				  					$this->where .= Crystal_Helper_Postgres::add_single_quote($v['value']);
							...
							{
								$this->where .= ' WHERE';
								$this->where .= ' ' . Crystal_Helper_Postgres::sanitize_string($parsed_params['column']);
				  				$this->where .= ' ' . $parsed_params['param'];
				  				$this->where .= Crystal_Helper_Postgres::add_single_quote($parsed_params['value']);
				...
				else
				{
					...
					foreach($params as $key => $value)
					{
			            ...
			            if($key == $first_element)
			            {
							...
							$this->where .= " WHERE " . Crystal_Helper_Postgres::sanitize_string($key)
							. " ="  . Crystal_Helper_Postgres::add_single_quote($value);
			            }
			            else
			            {
							$this->where .= " AND ". Crystal_Helper_Postgres::sanitize_string($key)
							 . " ="  . Crystal_Helper_Postgres::add_single_quote($value);
			            }
			...
			case 'and':
				if($first_element == '0')
				{
					if(isset($params[1]))
					{
						$this->where .= " AND " . Crystal_Helper_Postgres::sanitize_string($params[0])
						. " = "  . Crystal_Helper_Postgres::add_single_quote($params[1]);
			...
			case 'or':
				if($first_element == '0')
				{
						$this->where .= " OR " . Crystal_Helper_Postgres::sanitize_string($params[0]);
				}
				else
				{
					...
					$this->where .= " OR " . Crystal_Helper_Postgres::sanitize_string($key)
					. " = "  . Crystal_Helper_Postgres::add_single_quote($value);
			...
			case 'in':
				...
				foreach($params as $key => $value)
				{
					...
					{
						$this->where .= Crystal_Helper_Postgres::sanitize_string($value) .",";
					}
					else
					{
						$this->where .= Crystal_Helper_Postgres::sanitize_string($value);
			...
			case 'like':
				$this->where .= " LIKE ";
				$this->where .= Crystal_Helper_Postgres::add_single_quote("%" . $params[0] . "%") ;
			break;
